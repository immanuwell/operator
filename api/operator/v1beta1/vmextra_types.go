...
// DownsamplingConfig defines downsampling configuration for VMSingle
...
func (dc *DownsamplingConfig) validate(l *License) error {
	...
	if !l.IsProvided() {
		return fmt.Errorf("it is required to provide license key for downsampling. See [here](https://docs.victoriametrics.com/victoriametrics/enterprise/)")
	}
...
// as the VictoriaMetrics storage layer.
func (rf *RetentionFilter) validate() error {
	var ie promrelabel.IfExpression
	if err := ie.Parse(rf.Filter); err != nil {
		return fmt.Errorf("invalid filter %q: %w", rf.Filter, err)
	}
	var rd flagutil.RetentionDuration
	if err := rd.Set(rf.Retention); err != nil {
		return fmt.Errorf("invalid retention %q: %w", rf.Retention, err)
	}
	return nil
}

// validateRetentionFiltersAgainstPeriod checks that no filter retention exceeds the global retentionPeriod.
// prefix is used in error messages (e.g. "retentionFilters" or "vmstorage.retentionFilters").
// If retentionPeriod is empty the check is skipped.
func validateRetentionFiltersAgainstPeriod(filters []RetentionFilter, retentionPeriod, prefix string) error {
	if retentionPeriod == "" || len(filters) == 0 {
		return nil
	}
	var rpDur flagutil.RetentionDuration
	if err := rpDur.Set(retentionPeriod); err != nil {
		return fmt.Errorf("invalid retentionPeriod %q: %w", retentionPeriod, err)
	}
	for i, rf := range filters {
		var rfDur flagutil.RetentionDuration
		if err := rfDur.Set(rf.Retention); err != nil {
			return fmt.Errorf("%s[%d]: invalid retention %q: %w", prefix, i, rf.Retention, err)
		}
		if rfDur.Milliseconds() > rpDur.Milliseconds() {
			return fmt.Errorf("%s[%d]: retention %q must not exceed retentionPeriod %q", prefix, i, rf.Retention, retentionPeriod)
		}
	}
	return nil
}
